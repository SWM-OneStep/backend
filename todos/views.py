...
class TodoView(APIView):
    ...
    def post(self, request):
        ...
        last_todo = (
            Todo.objects.filter(
                user_id=data["user_id"], deleted_at__isnull=True
            )
            .order_by("-order")
            .first()
        )
    ...
    def patch(self, request):
        ...
        if not update_data and request.data.get("order") is None:
            return Response(
                {
                    "error": "update_data must contain at least one of\
                    content, category, start_date, or end_date"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if (
            ...
            and request.data.get("end_date")
            and update_data["start_date"] > update_data["end_date"]
        ):
            return Response(
                {"error": "start_date must be before end_date"},
                status=status.HTTP_400_BAD_REQUEST,
        ...
        if request.data.get("order"):
            ...
            if prev_id:
                prev = (
                    Todo.objects.filter(id=prev_id, deleted_at__isnull=True)
                    .first()
                    .order
                )
            if next_id:
                next = (
                    Todo.objects.filter(id=next_id, deleted_at__isnull=True)
                    .first()
                    .order
                )
            if (
                validate_order(prev=prev, next=next, updated=updated_order)
                is False
            ):
                return Response(
                    {"error": "Invalid order"},
...
class SubTodoView(APIView):
        ...
        for subtodo_data in data:
            ...
            last_subtodo = (
                SubTodo.objects.filter(
                    todo=subtodo_data["todo"], deleted_at__isnull=True
                )
                .order_by("-order")
                .first()
            )
    ...
    def patch(self, request):
        ...
        if not update_data and request.data.get("order") is None:
            return Response(
                {
                    "error": "At least one of content, date, or parent_id must be provided"  # noqa: E501
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        if "user_id" in request.data:
        ...
        serializer = SubTodoSerializer(
            sub_todo, data=update_data, partial=True
        )
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
...
class CategoryView(APIView):
    ...
    def patch(self, request):
        ...
        if not update_data:
            return Response(
                {"error": "At least one of title or color must be provided"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if "user_id" in request.data:
            return Response(
        ...
        serializer = CategorySerializer(
            category, data=update_data, partial=True
        )
        if serializer.is_valid(raise_exception=True):
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
