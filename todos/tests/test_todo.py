...
def test_get_todos_between_dates(create_user, create_category):
    ...
    response = client.get(url, {'user_id': create_user.id, 'start_date': '2024-08-03', 'end_date': '2024-08-06'}, format='json')
    assert response.status_code == 200
...
def test_delete_todo_success(create_user, create_category):
    ...
    assert Todo.objects.filter(id=todo.id, deleted_at__isnull=True).exists() == False
