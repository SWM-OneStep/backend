...
def test_get_subtodos_between_dates(create_todo):
    ...
    response = client.get(url, {'todo_id': create_todo.id, 'start_date': '2024-08-03', 'end_date': '2024-08-06'}, format='json')
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_update_subtodo_invalid_user_id(create_todo):
    subtodo = SubTodo.objects.create(
        content='Test SubTodo',
        date='2024-08-01',
        todo=create_todo,
...
def test_delete_subtodo_success(create_todo):
    ...
    assert SubTodo.objects.filter(id=subtodo.id, deleted_at__isnull=True).exists() == False
